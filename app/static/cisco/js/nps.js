function shouldHideSidebar() {
    if ($(window).width() < 992) {
        $('#npsSidebar').addClass('sidebar--hidden');
    }
}
function doNav(url) {
    shouldHideSidebar();
...
$(document).ready(function() {
    ...
    $('#npsHeader .toggle-menu').click(function() {
        $('#npsSidebar').toggleClass('sidebar--hidden');
    });

    // Wire the sidebar drawer open/close toggles
    $('#npsSidebar .sidebar__drawer > a').click(function() {
        $(this).parent().toggleClass('sidebar__drawer--opened');
    });

    // Wire the sidebar selected item
    $('#npsSidebar .sidebar__item > a').click(function() {
        $('#npsSidebar .sidebar__item').removeClass('sidebar__item--selected');
